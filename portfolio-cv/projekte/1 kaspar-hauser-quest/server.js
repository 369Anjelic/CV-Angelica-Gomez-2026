...
app.post('/api/profiles', (req, res) => {
  ...
  userProfiles.set(code, profile);
  userLogs.set(code, []);

  console.log(`✅ Profil erstellt: ${code} (${hero})`);
  res.json({ success: true, profile });
});

app.get('/api/profiles/:code', (req, res) => {
  const { code } = req.params;
  const profile = userProfiles.get(code);

  if (!profile) {
    return res.status(404).json({ error: 'Profile not found' });
  }

  res.json({ profile, logs: userLogs.get(code) || [] });
});

app.post('/api/profiles/:code/log', (req, res) => {
  const { code } = req.params;
  const { event, data } = req.body;

  if (!userProfiles.has(code)) {
    return res.status(404).json({ error: 'Profile not found' });
  }

  const log = { timestamp: new Date().toISOString(), event, data };

  if (!userLogs.has(code)) {
    userLogs.set(code, []);
  }

  userLogs.get(code).push(log);
